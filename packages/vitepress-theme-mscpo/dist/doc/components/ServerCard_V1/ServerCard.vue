  ...
  switch (localeIndex.value) {
    case "root":
      return {
        ...
        is_member: "成员服",
        is_member_desc:
          "此服务器是Minecraft服务器集体宣传组织的成员服，我们可以确保其可以长期运行",
        auth_mode: {
          official: "正版服",
          official_desc:
            "正版服务器要求玩家使用正版游戏客户端，并使用正版账号登录游戏。",
          yggdrasil: "外置登录",
          yggdrasil_desc:
            "外置登录要求玩家使用第三方正版验证服务器，使用第三方账户进行登录，具有正版的游玩体验。",
          offline: "离线服",
  ...
  >
    <article class="box" :class="[serverinfo.type]" @click="playSound">
      <div class="box-header" :title="serverinfo.name">
        ...
        />
        <div
          v-else-if="serverinfo.icon"
          class="icon"
          v-html="serverinfo.icon"
        ></div>
        <div class="info-container">
          <h4 class="ServerName" v-text="serverinfo.name" />
          <a class="ServerVersion"
            >{{ serverinfo.type }} {{ serverinfo.version }}
            <a
              v-if="serverinfo.ip"
              class="ServerVersion"
              :style="{ color: status_color }"
              v-html="status_text"
              v-show="server_status()"
            ></a
          ></a>
          <a-tooltip>
            <template #content>
      ...
      <a-tooltip v-if="serverinfo.desc">
        <template #content>
          ...
          <a-space wrap>
            ...
            <a-tooltip
              v-if="serverinfo.is_member"
              :content="i18nlang.is_member_desc"
            >
            ...
            <a-tooltip
              v-if="serverinfo.auth_mode == 'official'"
              :content="i18nlang.auth_mode.official_desc"
            >
              <a-tag color="gray" v-text="i18nlang.auth_mode.official"></a-tag>
            </a-tooltip>
            <a-tooltip
              v-if="serverinfo.auth_mode == 'offline'"
              :content="i18nlang.auth_mode.offline_desc"
            >
              <a-tag
                color="magenta"
                v-text="i18nlang.auth_mode.offline"
              ></a-tag>
            </a-tooltip>
            <a-tooltip
              v-if="serverinfo.auth_mode == 'yggdrasil'"
              :content="i18nlang.auth_mode.yggdrasil_desc"
            >
              <a-tag color="blue" v-text="i18nlang.auth_mode.yggdrasil"></a-tag>
            </a-tooltip>
            <a-tooltip
              v-for="tag in serverinfo.tags"
              v-if="serverinfo.tags"
              :content="i18nlang.tags.click + tag"
              @click.stop="copy(serverinfo.ip)"
            >
              <a-tag> {{ tag }}</a-tag>
            </a-tooltip>
          ...
          <div
            v-html="
              marked(
                serverinfo.desc ? serverinfo.desc.replace(/\n/g, '<br>') : ''
              )
            "
          ></div>
        </template>
