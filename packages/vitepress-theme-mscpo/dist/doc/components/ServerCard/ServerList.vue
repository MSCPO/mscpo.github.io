...
const fetchServers: RequestFunc<Server> = async ({ page, limit }) => {
  ...
  const filteredServers = shuffledServers().filter((server) => {
    ...
    const matchesType = servertype.value
    ...
    const matchesMember = is_member_option.value
      ...
      : true;
    const matchesTag = SelectedTags.value.length === 0 || serverTagsResult();
  ...
  if (ip != null) {
    try {
      const response = await alova.Get<StatusResponse>(
        `https://v2.mscpo.giize.com/?ip=${ip}`,
        {
          shareRequest: true,
        }
      );
    ...
    <div class="search">
      ...
      <div v-if="displayfilter" class="search-filters">
        ...
        >
          ...
          <a-radio value="Bedrock">{{
            i18nlang.filter.options.Bedrock
          }}</a-radio>
          <a-radio value="Netease">{{
            i18nlang.filter.options.Netease
          }}</a-radio>
        </a-radio-group>
        <br />
        {{ i18nlang.filter.tags }}:
        <a-checkbox v-model="StrictTag" @change="audio.play()"
          ><span v-text="i18nlang.filter.StrictTag"></span
        ></a-checkbox>
        <a-transfer
          :data="uniqueTagsData"
          ...
          simple
          :title="i18nlang.filter.TagSelectTitle"
          @change="audio.play()"
          show-search
        />
        <br />
        <div style="float: right; display: flex; gap: 5px">
          <a-button
            @click="filterclickbutton"
            v-text="i18nlang.filter.filterclean"
          ></a-button>
          <a-button
            type="primary"
            @click="filterbutton"
            v-text="i18nlang.filter.filter"
          ></a-button>
        </div>
      </div>
    </div>
